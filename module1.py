...
class VAE(nn.Module):
    ...
    def loss(
        ...
        kl_loss = 0.5 * torch.mean(kl_divergence_z) + 0.5 * torch.mean(kl_divergence_zG)
        # contrast_loss = 0.5 * torch.norm(inference_outputs['x1'] - inference_outputsG['x1'])
        # contrast_loss += 0.5 * torch.norm(inference_outputs['qz'].loc - inference_outputsG['qz'].loc)
        loss = reconst_loss + kl_loss # + contrast_loss
